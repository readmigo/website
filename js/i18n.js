(function () {
  ...
  function createFooterLangButton(code, container, isLast) {
    var btn = document.createElement('button');
    btn.setAttribute('data-lang', code);
    btn.className = 'hover:text-white transition text-xs';
    btn.textContent = LANG_LABELS[code];
    btn.addEventListener('click', function () {
      localStorage.setItem('readmigo-lang', code);
      applyLang(code);
  ...
  function buildFooterLangs() {
    ...
    container.textContent = '';
    for (var i = 0; i < SUPPORTED.length; i++) {
      createFooterLangButton(SUPPORTED[i], container, i === SUPPORTED.length - 1);
    }
  }
